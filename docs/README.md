# EM-Skill Documentation

Welcome to the EM-Skill documentation hub. This directory contains comprehensive guides for the fullstack engineering agent/skill/workflow system.

---

## 📚 Documentation Structure

### 🚀 Getting Started
- [Quick Start Guide](guides/getting-started.md) - Get up and running in 5 minutes
- [Usage Guide (English)](guides/usage-guide.md) - Comprehensive usage documentation
- [Hướng Dẫn Sử Dụng (Tiếng Việt)](vi/huong-dan-su-dung.md) - Hướng dẫn chi tiết bằng tiếng Việt

### 🏗️ Architecture
- [System Overview](architecture/overview.md) - High-level system architecture
- [Distributed System](architecture/distributed-system.md) - Distributed orchestration architecture
- [Component Design](architecture/components.md) - Component design and interactions

### 🔧 Protocols & Formats
- [Messaging Protocol](protocols/messaging.md) - Inter-agent communication
- [Report Format](protocols/report-format.md) - Agent report structure
- [Task Assignment](protocols/task-assignment.md) - Task delegation format

### 🔄 Workflows
- [Distributed Workflows](workflows/distributed.md) - Distributed execution workflows
- [Team Workflows](workflows/team.md) - Team collaboration workflows
- [Incident Response](workflows/incident.md) - Incident handling procedures
- [Security Workflows](workflows/security.md) - Security assessment workflows

### 📖 Skill System Guides
- [Skills Overview](skills/overview.md) - Available skills and when to use them
- [Agent Reference](agents/overview.md) - Agent documentation and reference
- [Workflow Reference](workflows/overview.md) - Complete workflow catalog

### 🧪 Testing
- [Test Suite](tests/README.md) - Test documentation and results
- [Manual Testing](tests/manual-testing.md) - Manual testing procedures

### 📝 Reference
- [CLI Reference](api/cli.md) - Command-line interface reference
- [Configuration](configuration.md) - Configuration options
- [Troubleshooting](troubleshooting.md) - Common issues and solutions

---

## 🎯 Quick Links

### For Beginners
1. Read [Quick Start Guide](guides/getting-started.md)
2. Try [Basic Examples](guides/usage-guide.md#examples)
3. Explore [Available Skills](skills/overview.md)

### For Advanced Users
1. Review [Distributed System Architecture](architecture/distributed-system.md)
2. Set up [Distributed Orchestration](guides/usage-guide.md#distributed-mode)
3. Configure [Custom Agents](agents/overview.md#custom-agents)

### For Contributors
1. Check [Project Structure](architecture/project-structure.md)
2. Follow [Contributing Guidelines](contributing.md)
3. Review [Code Standards](development/standards.md)

---

## 🌍 Language Support

Documentation is available in:
- 🇬🇧 **English** - See [guides/](guides/) directory
- 🇻🇳 **Tiếng Việt** - See [vi/](vi/) directory

---

## 📊 Documentation Metrics

| Section | Documents | Status |
|---------|-----------|--------|
| Getting Started | 3 docs | ✅ Complete |
| Architecture | 3 docs | ✅ Complete |
| Protocols | 3 docs | ✅ Complete |
| Workflows | 4 docs | ✅ Complete |
| Skills | 2 docs | ✅ Complete |
| Testing | 2 docs | ✅ Complete |
| Reference | 3 docs | ✅ Complete |

---

## 🔍 Finding Information

### By Role

**Developers:**
- Start with [Getting Started](guides/getting-started.md)
- Review [Skill Reference](skills/overview.md)
- Check [Workflow Guide](workflows/overview.md)

**Architects:**
- Read [System Overview](architecture/overview.md)
- Study [Distributed System](architecture/distributed-system.md)
- Review [Component Design](architecture/components.md)

**DevOps Engineers:**
- Check [Deployment Guide](guides/deployment.md)
- Review [CLI Reference](api/cli.md)
- See [Configuration](configuration.md)

**QA Engineers:**
- Read [Testing Guide](tests/README.md)
- Review [Test Suites](tests/overview.md)
- Check [Manual Testing](tests/manual-testing.md)

### By Task

**Setting up EM-Skill:**
→ [Getting Started Guide](guides/getting-started.md)

**Using skills:**
→ [Usage Guide](guides/usage-guide.md#using-skills)

**Running distributed investigations:**
→ [Distributed Workflows](workflows/distributed.md)

**Understanding protocols:**
→ [Messaging Protocol](protocols/messaging.md)

**Troubleshooting:**
→ [Troubleshooting Guide](troubleshooting.md)

---

## 📝 Contributing to Documentation

When contributing documentation:

1. **Follow the template** - Use existing docs as templates
2. **Be clear and concise** - Write for your audience
3. **Include examples** - Show, don't just tell
4. **Keep it updated** - Documentation should match code
5. **Use consistent formatting** - Follow Markdown best practices

### Documentation Guidelines

- **Language:** Write in clear, simple English
- **Tone:** Professional but approachable
- **Structure:** Use headings, lists, and code blocks
- **Examples:** Include real-world examples
- **Links:** Cross-reference related docs
- **Diagrams:** Use ASCII art for diagrams
- **Version:** Document version-specific changes

---

## 🔗 External Resources

- [Main README](../README.md) - Project overview
- [GitHub Repository](https://github.com/nv-minh/agent-team) - Source code
- [Feature Branches Summary](../FEATURE-BRANCHES-SUMMARY.md) - Feature branches
- [Merge Summary](../MERGE-SUMMARY.md) - Merge details

---

## 📞 Support

Need help?

- 📖 Read the [Troubleshooting Guide](troubleshooting.md)
- 💬 Check [GitHub Issues](https://github.com/nv-minh/agent-team/issues)
- 📧 Contact maintainers

---

**Last Updated:** 2026-04-19
**Documentation Version:** 1.0.0
**Maintained By:** EM-Skill Team
